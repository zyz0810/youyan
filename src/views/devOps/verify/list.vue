  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" :model="listQuery" class="search_form">
        ...
        <el-form-item label="">
          <el-select v-model="listQuery.status" placeholder="故障类型" @change="handleFilter">
            <el-option label="启用" value="1"></el-option>
            <el-option label="禁用" value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="">
          <el-select v-model="listQuery.status" placeholder="核实状态" @change="handleFilter">
            <el-option label="启用" value="1"></el-option>
            <el-option label="禁用" value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="">
          <el-select v-model="listQuery.status" placeholder="修复状态" @change="handleFilter">
            <el-option label="启用" value="1"></el-option>
            <el-option label="禁用" value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="" prop="name">
          <el-input v-model.trim="listQuery.name" placeholder="设备编号" @change="handleFilter" clearable/>
        </el-form-item>
        <el-form-item label="" prop="name">
          <el-input v-model.trim="listQuery.name" placeholder="时间" @change="handleFilter" clearable/>
        </el-form-item>
        <el-form-item label="" prop="name">
    ...
    <el-table v-loading="listLoading" :data="list" :height="tableHeight" element-loading-text="拼命加载中" fit ref="tableList">
      ...
      <el-table-column label="故障类型" align="center" prop="address"></el-table-column>
      <el-table-column label="故障日期" align="center" width="140">
        <template slot-scope="scope">
          <span>{{$moment(scope.row.time).format('YYYY-MM-DD HH:mm:ss')}}</span>
        </template>
      </el-table-column>
      <el-table-column label="核实状态" align="center" prop="num">
        <template slot-scope="scope">
          <span>{{$moment(scope.row.time).format('YYYY-MM-DD HH:mm:ss')}}</span>
        </template>
      </el-table-column>
      <el-table-column label="修复状态" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" min-width="160">
        <template slot-scope="scope">
          <el-button class="filter-item" type="primary" @click="handleUpdate">核实</el-button>
        </template>
      </el-table-column>
    </el-table>
